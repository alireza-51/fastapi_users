...
async def test_create_user(client, db_session):
    response = client.post("api/users/", json={"username": "newuser", "password": "newpassword", "email": "newemail@example.com"})
...
async def test_update_user(client, auth_token, admin_token):
    ...
    response = client.put(f"api/users/{auth_token['user'].id}", headers=headers, json={"username": "newuser", "password": "newpassword", "email": "newemail@example.com"})
    assert response.status_code == status.HTTP_200_OK
    
    # Test admin can update any user
    response = client.put(f"/users/{auth_token['user'].id}", headers=admin_headers, json={"username": "newuser", "password": "newpassword", "email": "newemail@example.com"})
    assert response.status_code == status.HTTP_200_OK
